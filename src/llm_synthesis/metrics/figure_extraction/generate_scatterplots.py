...
import llm_synthesis.metrics.figure_extraction.resources as resources
...
def random_x_axis(ltype: float) -> str:
    return random.choice(
        resources.filler_axis_labels
        if ltype >= 0.4
        else resources.x_axis_labels
    )


def random_filler() -> str:
    return random.choice(resources.filler_list)


def random_matrix() -> str:
    return random.choice(resources.matrix_list)


def skewed_normal_distribution(
    num_groups: int, center: int = 6, skew: int = -1, size: int = 1
) -> int:
    """
    Generate a skewed normal distribution for the number of points in each group.
    Args:
    ...
    if num_groups <= 2:
        center += 2
    return int(
        np.clip(skewnorm.rvs(a=skew, loc=center, scale=2, size=size), 3, 20)[0]
    )


def skewed_marker_size(
...
def generate_x_points(num_points: int) -> np.ndarray:
    """Generate x points for the scatter plot.
    Args:
        num_points (int): Number of points to generate.

    Returns:
        np.ndarray: Sorted x points, either uniformly distributed or randomly selected.
    """
    return (
...
def pick_color(used_colors: set[str]) -> str:
    available_colors = set(resources.colors) - used_colors
    color = random.choice(list(available_colors))
    return color


def pick_shape(used_shapes: set[str]) -> str:
    available_shapes = set(resources.shapes) - used_shapes
    shape = random.choice(list(available_shapes))
    return shape


def generate_legend_label(
    ...
    if legend_type < 0.2:
        ...
        return f"{random_filler()}/{default_mat}"
    elif legend_type < 0.8:
        return random_matrix()
    return None


def adjust_y_axis(ax: plt.Axes) -> None:
...
def place_legend(
    ...
    """
    Place the legend in the best position to avoid overlap with scatter points.
    It iterates through predefined positions and selects the one with the least overlap.
    If no position is suitable, it defaults to the upper right corner.
    """
    best_pos = "upper right"
    min_overlap = float("inf")
    for pos in resources.positions:
        legend = ax.legend(
...
def plot_random_scatter_on_ax(ax: plt.Axes) -> dict[str, list[list[float]]]:
    """
    ...
    x = generate_x_points(num_points) if consistent_x else None

    legend_type = random.random()
    marker_size = skewed_marker_size()
    default_mat = random_matrix()
    default_fill = random_filler()
    x_label = random_x_axis(legend_type)
    y_label = random.choice(resources.y_axis_labels)
    ...
    for _ in range(num_groups):
        group_x = x if consistent_x else generate_x_points(num_points)
        group_x, group_y = generate_random_data(
            group_x, random.choice(curve_types)
        )

        color = pick_color(used_colors)
        shape = pick_shape(used_shapes)
        ...
        label = generate_legend_label(legend_type, default_fill, default_mat)
        if label:
...
def plot_multiple_subplots(image_path: str, groundtruth_path: str) -> None:
    ...
    for ax in axes_list:
        subplot_labels = plot_random_scatter_on_ax(ax)
...
def generate_synthetic_plots(
    num_plots: int, images_path: str, groundtruths_path: str, seed: int = 42
) -> None:
    """
    Generate synthetic scatter plots and save them along with their ground truth coordinates.
    Args:
        num_plots (int): Number of plots to generate.
        images_path (str): Directory to save the generated images.
        groundtruths_path (str): Directory to save the ground truth coordinates
                                under the format {name_of_group: [[x1, y1], [x2, y2], ...]}.
    ...
    for i in range(num_plots):
        ...
        groundtruth_path = os.path.join(
            groundtruths_path, image_name.replace("png", "csv")
        )
        plot_multiple_subplots(image_path, groundtruth_path)
