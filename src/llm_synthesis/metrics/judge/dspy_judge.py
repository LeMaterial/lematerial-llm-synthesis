...
class SynthesisJudgeSignature(dspy.Signature):
    """
    You are an expert materials scientist tasked with evaluating an LLM-extracted synthesis recipe against a reference synthesis procedure.

    Evaluate the extracted recipe according to the following criteria on a scale of 1-5 (1.0: poor, 5.0: excellent, 0.5 step increments):

    1. Materials Appropriateness: Are the selected materials suitable for the target synthesis?
    2. Equipment Appropriateness: Is the selected equipment suitable for the synthesis?
    3. Procedure Completeness: Are all necessary steps included with sufficient detail?
    4. Procedure Similarity: How closely does the procedure match the reference?
    5. Procedure Feasibility: Can this procedure be realistically executed in a lab?
    6. Characterization Appropriateness: Are the characterization metrics suitable for validating success?
    7. Characterization Similarity: How well do predicted properties match the reference results?

    Provide detailed reasoning for each individual score, then calculate an overall score as the average of all criteria.
...
def make_dspy_synthesis_judge_signature(
    ...
    instructions: str = "Evaluate an LLM-extracted synthesis recipe against a reference synthesis procedure from the perspective of an expert materials scientist.",
    target_material_description: str = "A description of the target material to be synthesized.",
    extracted_recipe_description: str = "The extracted synthesis recipe.",
    synthesis_procedure_description: str = "The reference synthesis procedure from a scientific publication.",
    evaluation_description: str = "The structured evaluation containing detailed reasoning and scores.",
) -> type[dspy.Signature]:
    """
    ...
    Args:
        ...
        target_material_description (str): Description for the target material input.
        extracted_recipe_description (str): Description for the extracted recipe input.
        synthesis_procedure_description (str): Description for the synthesis procedure input.
