...
from llm_synthesis.services.pdf_extraction.base_pdf_extractor import (
    BasePDFExtractor,
)
...
class DoclingPDFExtractor(BasePDFExtractor):
    ...
    def extract_to_markdown_with_figures_embedded(self, pdf_data: bytes) -> str:
        """
        ...
        result = conv.convert(DocumentStream(name="pdf", stream=io.BytesIO(pdf_data)))
        doc = result.document
