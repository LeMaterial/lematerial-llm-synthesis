...
def sigmoid_curve(T, T50, k, y_max=100):
    """Generate a sigmoid-shaped conversion curve."""
    return y_max / (1 + np.exp(-k * (T - T50)))

def make_fig1():
    np.random.seed(42)
    T = np.linspace(300, 700, 80)

    catalysts = [
        # (label, metal_key, support, T50, steepness, max_conv, paper)
    ...
    for label, mk, sup, T50, k, ymax, paper in catalysts:
        y = sigmoid_curve(T, T50, k, ymax) + np.random.normal(0, 0.8, len(T))
        y = np.clip(y, 0, 100)
        color = PAL[mk]
        marker = SUPPORT_MARKERS.get(sup, "o")
        ax.plot(T, y, color=color, alpha=0.75, linewidth=1.6)
        # mark every 15th point
        ax.scatter(T[::15], y[::15], color=color, marker=marker,
                   s=30, zorder=3, edgecolors="white", linewidth=0.4)
    ...
    # ── Legend: metals ──
    metal_handles = [Line2D([0], [0], color=PAL[m], lw=2.5, label=m)
    ...
    # ── Legend: supports ──
    sup_handles = [Line2D([0], [0], marker=SUPPORT_MARKERS[s], color="grey",
                          lw=0, markersize=7, label=s)
                   for s in ["SiO₂", "CeO₂", "MgO", "CaO", "Al₂O₃"]]
    ax.legend(handles=sup_handles, title="Support",
              loc="center right", frameon=True, framealpha=0.9,
              edgecolor="grey", bbox_to_anchor=(1.0, 0.55))

    ax.set_xlabel("Temperature (°C)")
    ax.set_ylabel("NH₃ Conversion (%)")
    ax.set_title("Figure 1 · Cross-Paper NH₃ Conversion Landscape", fontweight="bold")
    ax.set_xlim(300, 700)
    ax.set_ylim(0, 105)
...
# ══════════════════════════════════════════════════════════════════════════
# FIGURE 2 — Metal × Support Heatmap ("Catalyst Periodic Table")
# ══════════════════════════════════════════════════════════════════════════
...
def make_fig2():
    ...
    known = {
        (0,0): 99, (0,1): 98, (0,2): 82, (0,5): 90,         # Ru
        (1,2): 87, (1,3): 82, (1,4): 84,                      # Ni
        (2,2): 78, (2,3): 75,                                  # Co
        (3,2): 70,                                              # Fe
        (4,3): 91, (4,2): 80,                                  # Ni-Co
        (5,2): 86,                                              # Fe-Co
    ...
    ax.set_title("Figure 2 · Best NH₃ Conversion at 500 °C by Metal–Support Combination",
                  fontweight="bold")
    ...
    # ── Annotate unexplored ──
    ax.annotate("grey = unexplored\ncombination", xy=(5.5, 6.5),
...
def make_fig3():
    ...
    sequences = [
        ...
        ["dissolve", "disperse", "add\nprecursor", "sonicate", "stir", "evaporate", "dry", "sieve", "reduce"],
        ["dissolve", "disperse", "add\nprecursor", "sonicate", "stir", "evaporate", "dry", "sieve", "reduce"],
        # Fang 2023 - hydrothermal + impregnation
        ["dissolve", "stir", "precipitate", "hydrothermal", "wash", "dry", "calcine", "impregnate", "stir", "dry", "heat\ntreat"],
        # Sayas 2020
        ["dissolve", "impregnate", "dry", "calcine", "add\npromoter", "dry", "reduce"],
        # Do 2024
        ["dissolve", "add\nprecursor", "stir", "precipitate", "wash", "dry", "calcine", "reduce"],
        # Gao 2023 - coprecipitation
        ["dissolve", "add\nprecursor", "coprecipitate", "wash", "dry", "calcine", "reduce"],
        # Lucentini 2019
        ["dissolve", "impregnate", "stir", "dry", "calcine", "reduce"],
        # Hu 2024
        ["dissolve", "add\nprecursor", "stir", "precipitate", "age", "wash", "dry", "calcine", "impregnate", "dry", "reduce"],
        # Maleki 2024
    ...
    # ── Node sizes & colors ──
    node_sizes = [node_counts[n] * 220 + 200 for n in G.nodes()]
    node_colors_vals = [node_counts[n] for n in G.nodes()]
    ...
    # ── Edge widths ──
    edge_widths = [edge_counts[(u, v)] * 1.5 + 0.3 for u, v in G.edges()]
    edge_colors = [f"#{int(180 - edge_counts[(u,v)]*15):02x}{int(180 - edge_counts[(u,v)]*15):02x}{int(180 - edge_counts[(u,v)]*15):02x}"
                   for u, v in G.edges()]
    ...
    edge_labels = {(u, v): str(w) for (u, v), w in edge_counts.items() if w >= 3}
    nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, ax=ax,
                                  font_size=7, font_color="#E07A5F",
    ...
    ax.set_title("Figure 3 · Synthesis Action Network — Sequential Steps across 12 Papers",
...
def make_fig4():
    ...
    for idx, (name, values) in enumerate(catalysts_data.items()):
        ...
        ax.scatter(angles[:-1], norm_vals[:-1], color=colors[idx], s=40, zorder=5)
        ...
        ax.set_title(name, fontsize=9, fontweight="bold", pad=18,
                     color=colors[idx])
        ax.grid(alpha=0.3)

    fig.suptitle("Figure 4 · Synthesis Parameter–Performance Radar Charts\nfor Top NH₃ Decomposition Catalysts",
...
def make_fig5():
    ...
    prom_colors = {"K": "#2176AE", "La": "#E07A5F", "Ce": "#81B29A",
                   "Nd": "#F2CC8F", "Sm": "#3D405B", "Ca": "#D4A017"}
    ...
    # Base bars (light)
    ax1.barh(y_pos, base_vals, height=0.5, color="#dddddd",
             edgecolor="white", label="Unpromoted")
    # Promoted extension
    ax1.barh(y_pos, deltas, left=base_vals, height=0.5,
             color=bar_colors, edgecolor="white", alpha=0.85)

    for i, (b, p, d) in enumerate(zip(base_vals, promoted_vals, deltas)):
        ax1.text(p + 0.5, i, f"+{d}%", va="center", fontsize=9,
    ...
    conversion = 30 + 50 * np.exp(-0.003 * (red_temp - 450)**2 / 50) + np.random.normal(0, 5, n)
    ...
    scatter = ax2.scatter(calc_temp, red_temp,
                          c=conversion, cmap="RdYlGn",
                          s=metal_load * 25 + 30,
                          edgecolors="white", linewidth=0.6,
                          vmin=40, vmax=100, alpha=0.85, zorder=3)
    ...
    for ml, lab in [(2, "2 wt%"), (8, "8 wt%"), (15, "15 wt%")]:
        ax2.scatter([], [], s=ml * 25 + 30, c="grey", alpha=0.5,
                    edgecolors="white", label=lab)
    ax2.legend(title="Metal Loading", loc="upper left", fontsize=8,
    ...
    fig.suptitle("Figure 5 · Promoter Effects & Synthesis–Performance Correlations",
                  fontweight="bold", fontsize=14, y=1.02)
    fig.tight_layout()
