...
def find_matching_text_tc_multi(material: str, tc_entries: list[dict]) -> list[dict]:
    ...
    matches.sort(key=lambda x: (not x.get("superconducting", False), -(x.get("Tc_mid") or 0)))
    return matches
...
def crop_bottom_left_quadrant(img_base64: str) -> str:
    """Crop bottom-left quadrant -> base64 PNG.  ~4x resolution in SC transition region."""
...
CSV_COLUMNS = [
    ...
    "tc_vlm_orig", "tc_vlm_orig_onset", "tc_vlm_orig_zero", "tc_vlm_orig_source",
    # VLM Tc — snippet (full + bottom-left crop)
    "tc_vlm_snip", "tc_vlm_snip_onset", "tc_vlm_snip_zero", "tc_vlm_snip_source",
    "tc_vlm_source_plot",
    # Best Tc
...
If the exact method is not in the list, use the closest match or 'other'."""
    ...
    n_conditions = len(set((e["material"], e["condition"]) for e in tc_from_text))
    print(f"  Found {n_text_tc} Tc values across {n_conditions} (material, condition) pairs")
    ...
    if not skip_figures:
        ...
        if figures:
            ...
            for pi, p in enumerate(plots):
                print(f"    Plot {pi}: x='{p.x_axis_label}' [{p.x_axis_unit}]  "
            ...
            if relevant_plots:
                ...
                def _call_vlm(fig_b64: str, prompt: str, crop_b64: str | None = None) -> tuple[str, float]:
                    ...
                    content.append({"type": "image", "source": {
                        "type": "base64", "media_type": _img_type(fig_b64), "data": fig_b64,
                    }})
                    if crop_b64:
                        content.append({"type": "text", "text": "Image 2 — ZOOMED bottom-left quadrant (low T, low R region):"})
                        content.append({"type": "image", "source": {
                            "type": "base64", "media_type": _img_type(crop_b64), "data": crop_b64,
                        }})
                ...
                print(f"\n  VLM cost: Original ${_cost_original:.4f} | Snippet ${_cost_snippet:.4f}"
    ...
    # Collect unmatched VLM series (have Tc but weren't linked to any Step 1 material)
    linked_series = set()
    for mapping in plot_mappings:
        for sm in mapping.mappings:
    ...
    for name in list(vlm_tc_per_material_orig.keys()) + list(vlm_tc_per_material_snip.keys()):
        all_vlm_material_norms.add(_normalize_formula(name))

    for plot_idx in set(list(tc_from_vlm_original.keys()) + list(tc_from_vlm_snippet.keys())):
        orig_results = tc_from_vlm_original.get(plot_idx, {})
        snip_results = tc_from_vlm_snippet.get(plot_idx, {})
        all_series_names = set(list(orig_results.keys()) + list(snip_results.keys()))
    ...
    if vlm_only_materials:
        print(f"  Found {len(vlm_only_materials)} additional materials from VLM Tc "
    ...
    if text_only_materials:
        print(f"  Found {len(text_only_materials)} additional materials from text Tc "
    ...
    for material in all_materials:
        ...
        result = {
            ...
            } if vlm_snip else None,
            "performance": performance_data[material].model_dump() if material in performance_data else None,
        }
    ...
    summary = {
        ...
        "materials_with_text_tc": sum(
            1 for m in all_materials
            if any(e.get("Tc_mid") is not None for e in text_tc_per_material.get(m, []))
        ),
        "materials_with_vlm_tc_orig": sum(1 for v in vlm_tc_per_material_orig.values() if _vlm_data_has_tc(v)),
        "materials_with_vlm_tc_snip": sum(1 for v in vlm_tc_per_material_snip.values() if _vlm_data_has_tc(v)),
        "total_text_conditions": sum(len(v) for v in text_tc_per_material.values()),
    }
    ...
    for material in all_materials:
        ...
        vlm_orig_sc = vlm_orig.get("superconducting")
        vlm_orig_source = vlm_orig.get("source", "main plot") if vlm_orig else None

        vlm_snip_tc = vlm_snip.get("tc_mid")
        vlm_snip_onset = vlm_snip.get("t_onset")
        vlm_snip_zero = vlm_snip.get("t_zero")
        vlm_snip_sc = vlm_snip.get("superconducting")
        vlm_snip_source = vlm_snip.get("source", "main plot") if vlm_snip else None
        ...
        if text_entries:
            for te in text_entries:
                ...
                flat_records.append({
                    "paper_id": paper.id, "year": year,
                    "material": material,
                    "material_normalized": normalize_formula_for_csv(material),
                    "condition": condition,
                    "is_superconductor": is_sc,
                    "tc_text": text_tc, "tc_text_onset": text_onset,
                    "tc_text_zero": text_zero, "tc_text_source": None,
                    "tc_vlm_orig": vlm_orig_tc, "tc_vlm_orig_onset": vlm_orig_onset,
                    "tc_vlm_orig_zero": vlm_orig_zero, "tc_vlm_orig_source": vlm_orig_source,
                    "tc_vlm_snip": vlm_snip_tc, "tc_vlm_snip_onset": vlm_snip_onset,
                    "tc_vlm_snip_zero": vlm_snip_zero, "tc_vlm_snip_source": vlm_snip_source,
            ...
            flat_records.append({
                ...
                "condition": "ambient",
                "is_superconductor": is_sc,
                "tc_text": None, "tc_text_onset": None,
                "tc_text_zero": None, "tc_text_source": None,
                "tc_vlm_orig": vlm_orig_tc, "tc_vlm_orig_onset": vlm_orig_onset,
                "tc_vlm_orig_zero": vlm_orig_zero, "tc_vlm_orig_source": vlm_orig_source,
                "tc_vlm_snip": vlm_snip_tc, "tc_vlm_snip_onset": vlm_snip_onset,
                "tc_vlm_snip_zero": vlm_snip_zero, "tc_vlm_snip_source": vlm_snip_source,
...
def append_to_master_csv(flat_records: list[dict], master_path: Path):
    ...
    if master_path.exists() and master_path.stat().st_size > 0:
        with open(master_path, "r", newline="") as f:
            ...
            for row in reader:
                existing_keys.add((row.get("paper_id", ""), row.get("material", ""), row.get("condition", "")))

    new_keys = {(r["paper_id"], r["material"], r.get("condition", "")) for r in flat_records}
        ...
        if master_path.exists():
            with open(master_path, "r", newline="") as f:
                ...
                all_rows = [row for row in reader
                            if (row.get("paper_id", ""), row.get("material", ""), row.get("condition", "")) not in replace_keys]
...
def main():
    ...
    for i, pdf_path in enumerate(pdfs, 1):
        ...
        try:
            ...
            results_log.append({
                ...
                "records": len(flat_records), "with_tc": n_tc,
                "time_s": f"{elapsed:.0f}",
            })
            ...
            results_log.append({
                ...
                "records": 0, "with_tc": 0, "time_s": f"{elapsed:.0f}",
            })
    ...
    for r in results_log:
        status = r["status"][:8]
        print(f"{r['paper']:<45} {status:<10} {r['records']:>5} {r['with_tc']:>5} {r['time_s']:>5}s")
