"""Compare multi-LLM result.json with result_human.json and generate agreement statistics
...
from eval_utils import (  # pylint: disable=wrong-import-position
    SCORE_COLUMNS, col_label, compute_agreement_metrics, merge_on_material_id,
)
...
def _log_full_row(label, metrics):
    ...
    logging.info(
        ...
        label, metrics["rho"], metrics["p"], metrics["kappa"],
        metrics["icc2"], metrics["icc3"],
        metrics["h_mean"], metrics["h_median"], metrics["h_std"],
        metrics["l_mean"], metrics["l_median"], metrics["l_std"], metrics["n"],
    )
...
def _log_compact_row(label, metrics):
    ...
    logging.info(
        ...
        label, metrics["rho"], metrics["kappa"],
        metrics["h_mean"], metrics["l_mean"],
        metrics["abs_diff"], metrics["n"],
    )
...
def analysis_by_score_category(human_df, llm_df, score_cols):
    ...
    for col in score_cols:
        metrics = compute_agreement_metrics(merged[f"{col}_h"], merged[f"{col}_l"])
        if metrics:
...
def analysis_by_group(human_df, llm_df, score_cols, group_col, group_label):
    ...
    for value in group_values:
        if group_col == "synth_llm":
            ...
            merged = merge_on_material_id(human_df, llm_df[llm_df["judge_id"] == value], [OVERALL_COL])
...
def per_score_per_judge(human_df, llm_df, score_cols):
    ...
    for judge in sorted(llm_df["judge_id"].dropna().unique()):
        merged = merge_on_material_id(human_df, llm_df[llm_df["judge_id"] == judge], score_cols)
        logging.info("\n--- Judge: %s ---", judge)
        logging.info("%s\n%s", COMPACT_HEADER, "-" * len(COMPACT_HEADER))
        for col in score_cols:
            metrics = compute_agreement_metrics(merged[f"{col}_h"], merged[f"{col}_l"])
...
def analyze_by_material_category(human_df, llm_df, score_cols, cat_col, cat_label):
    ...
    for cat_value in cat_values:
        ...
        for judge in judge_names:
            merged = merge_on_material_id(h_cat, l_cat[l_cat["judge_id"] == judge], [OVERALL_COL])
            metrics = compute_agreement_metrics(
                merged[f"{OVERALL_COL}_h"], merged[f"{OVERALL_COL}_l"]
            )
            if metrics:
                ...
                csv_rows.append({
                    "category_type": cat_label, "category": cat_value,
                    "judge": judge, "n": metrics["n"],
                    "rho": metrics["rho"], "p": metrics["p"],
                    "kappa": metrics["kappa"],
                    "icc2": metrics["icc2"], "icc3": metrics["icc3"],
                    "h_mean": metrics["h_mean"], "h_median": metrics["h_median"],
                    "h_std": metrics["h_std"],
                    "l_mean": metrics["l_mean"], "l_median": metrics["l_median"],
                    "l_std": metrics["l_std"],
                    "mean_diff": metrics["mean_diff"], "abs_diff": metrics["abs_diff"],
                })
            else:
        ...
        if best_judge:
            logging.info("    >> Best judge: %s (AbsDiff=%.3f)", best_judge, best_abs)

        # Per-judge per-score-column detail
        for judge in judge_names:
            merged = merge_on_material_id(h_cat, l_cat[l_cat["judge_id"] == judge], score_cols)
            if merged.empty:
                continue
            logging.info("\n      [%s / %s]", cat_value, judge)
            for col in score_cols:
                m = compute_agreement_metrics(merged[f"{col}_h"], merged[f"{col}_l"])
                if m:
                    logging.info(
                        "        %-28s Rho=%6.3f Kappa=%6.3f AbsDiff=%6.3f n=%d",
                        col_label(col), m["rho"], m["kappa"], m["abs_diff"], m["n"],
                    )

    # Summary: best judge per category
    if grid:
        _section(f"BEST JUDGE PER {cat_label} (lowest AbsDiff on {OVERALL_COL})")
        header = f"{'Category':<35} {'Best Judge':<30} {'AbsDiff':>7} {'Rho':>6} {'n':>4}"
        logging.info("%s\n%s", header, "-" * len(header))
        for cat_value in sorted(grid):
            cat_metrics = grid[cat_value]
            best_j = min(cat_metrics, key=lambda j, cm=cat_metrics: cm[j]["abs_diff"])
            best = grid[cat_value][best_j]
            logging.info(
                "%-35s %-30s %7.3f %6.3f %4d",
                cat_value, best_j, best["abs_diff"], best["rho"], best["n"],
            )
...
def _make_category_heatmap(grid, cat_values, judge_names, cat_label):
    ...
    sns.heatmap(
        ...
        xticklabels=judge_names, yticklabels=cats_with_data,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Compare multi-LLM results -- by category")
    ...
    analysis_by_group(human_df, llm_df, score_cols, "synth_llm", "SYNTHESIS LLM")
    ...
    for cat_col, cat_label in [
        ...
        rows = analyze_by_material_category(human_df, llm_df, score_cols, cat_col, cat_label)
        csv_rows.extend(rows)

    if csv_rows:
        csv_path = os.path.join(OUTPUT_DIR, "multi_llm_agreement_by_material_category.csv")
